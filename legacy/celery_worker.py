...
def init_worker(**kwargs):
    ...
    global AsyncSessionLocal  # Overwrite the global session maker with one bound to new_engine
    AsyncSessionLocal = async_sessionmaker(
        bind=new_engine, class_=AsyncSession, expire_on_commit=False
    )
    print("Worker process initialized a new async engine!")  # A little friendly message
